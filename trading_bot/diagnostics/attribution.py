...
def volatility_bucket(vol_state: float) -> str:
...
def attribution_groups(trade_contexts: list[dict[str, Any]]) -> dict[str, list[tuple[str, float]]]:
    ...
    for t in trade_contexts:
        ...
        groups["volatility_regime"].append((volatility_bucket(t.get("vol_state", math.nan)), pnl))
